...
**Status:** Phases 0–1 complete; Phase 2 implementation complete with all local gates passing — the compatibility bridge deployment and soak remain before Phase 2 closes and Phase 3 begins
...
Open question Q4 (§17) — whether the hosted closed relay requires periodic auth-tag refresh independent of key rotation — feeds directly into this procedure and must be closed before the first production Buzz endpoint.
...
| --- | ------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Q4  | Does the hosted closed relay require auth-tag refresh/rotation independent of key rotation? | Phase 4 — must close **before the first production Buzz endpoint**, not before Phase 0 | assume no automatic refresh; §14.5 rotation is manual; alert on auth failure                                                                                               |
...
Ground-truth references verified against the repository at this revision: `src/config/schema.ts` (v1 schema, `SECRET_PATHS`, `superRefine` invariants), `src/agent-api/pool.ts` (`SideSessionPool`, per-token caps), `src/agent-api/errors.ts` (429 codes), `src/runner/types.ts` (`SIDE_SESSION_ID_REGEX`, `runnerSupportsSideSessions`), `src/runner/claude-code.ts` (`claudeUuid`, `--session-id`), `src/runner/codex.ts` (one-shot `exec resume` model), `src/db/schema.sql`, `src/db/migrations/0001–0003`, `src/doctor.ts` (C001–C015).
...
| Buzz auth-tag refresh semantics are unknown (Q4)                                                                                             | High     | Manual rotation per §14.5 plus fail-closed alerting on auth failure, until the relay contract is confirmed                                                                                                                            |
...
| Buzz protocol drift — kinds and tags may change upstream                                                                                     | Low      | Centralized in `platform/buzz/kinds.ts`; unknown kinds are counted and ignored, never fatal (§10.1)                                                                                                                                   |
...
- **Phase 2 implementation pass:** added validated config v2 plus lossless v1 normalization and upgrade output; added schema v5, sanitized migration planning, automatic snapshots, incremental auto-vacuum, normalized DB helpers, compatibility dual-writes, and string-safe platform-neutral outbox delivery; rehearsed bridge → v2 → bridge and emergency snapshot restoration; and verified v1 parity plus a complete v2 Telegram flow. All local gates pass with 1,305 tests passed and 13 intentionally skipped. The Railway bridge deployment and schema-v3/schema-v5 soaks remain the final Phase 2 gate, so Phase 3 is not yet authorized to begin.
